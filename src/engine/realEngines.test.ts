...
describe('real engine registry', () => {
  ...
  it('uses hardware-conscious quality profiles for the advanced engines', () => {
    ...
    expect(engineRequestProfile('hunyuan-2.1', 'high')).toMatchObject({ resolution: 512, inferenceSteps: 50, targetTriangles: 100000, textureSize: 2048, paintProfile: 'full' })
    expect(engineRequestProfile('hunyuan-2.1', 'high', 'shape-only')).toMatchObject({ resolution: 512, inferenceSteps: 50, textureSize: 0, paintProfile: 'full' })
